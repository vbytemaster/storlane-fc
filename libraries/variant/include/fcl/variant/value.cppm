...
export namespace fcl {
   ...
   public:
      ...
      BOOST_DESCRIBE_NESTED_ENUM(type_id, null_type, int64_type, uint64_type, double_type, bool_type, string_type, array_type, object_type, blob_type)
   ...
   public:
      ...
      mutable_variant_object operator()(const mutable_variant_object& mvo) &&;
      explicit mutable_variant_object(variant v) : _key_value(new std::vector<entry>()) { *this = v.get_object(); }
      template<typename T,
      ...
      template<typename T> mutable_variant_object(std::string key, const std::initializer_list<T>& val) : _key_value(new std::vector<entry>()) {
         set(std::move(key), variant(val));
      }
