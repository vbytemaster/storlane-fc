...
sys_seconds from_iso_seconds(std::string_view value) {
   try {
      ...
      throw std::invalid_argument("unable to convert ISO-formatted string to std::chrono::sys_seconds: " + std::string{value});
   }
}
...
sys_time_us from_iso_time_point(std::string_view value) {
   try {
      ...
      if (dot == std::string::npos) {
         return sys_time_us{std::chrono::duration_cast<std::chrono::microseconds>(
            from_iso_seconds(text).time_since_epoch())};
      }
      ...
      const auto base = sys_time_us{std::chrono::duration_cast<std::chrono::microseconds>(
         from_iso_seconds(text).time_since_epoch())};
      return base + std::chrono::milliseconds{to_int64(millis) - 1000};
   } catch (const std::exception&) {
      throw std::invalid_argument("unable to convert ISO-formatted string to std::chrono::sys_time<microseconds>: " + std::string{value});
   }
}

std::string get_approximate_relative_time_string(
...
std::string get_approximate_relative_time_string(
   ...
   return get_approximate_relative_time_string(
      std::chrono::time_point_cast<std::chrono::seconds>(event_time),
