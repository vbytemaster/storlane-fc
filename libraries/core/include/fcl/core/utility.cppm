...
export module fcl.core.utility;
...
namespace detail {
template <typename T> fcl::true_type is_class_helper(void (T::*)());
template <typename T> fcl::false_type is_class_helper(...);
...
// outside of namespace fcl becuase of VC++ conflict with std::swap
export template <typename T> void fcl_swap(T& a, T& b) {
   T tmp = std::move(a);
   a = std::move(b);
   b = std::move(tmp);
}
