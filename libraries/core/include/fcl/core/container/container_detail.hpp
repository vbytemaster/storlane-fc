...
namespace fcl {
      ...
      namespace detail {
         ...
         inline void unpack_set( Stream& s, Set<T, U...>& value ) {
            ...
            for( uint32_t i = 0; i < size.value; ++i ) {
               auto tmp = ::fcl::detail::construct_maybe_with_allocator<T>( value.get_allocator() );
               unpack( s, tmp );
         ...
         inline void unpack_map( Stream& s, Map<K, V, U...>& value ) {
            ...
            value.clear();
            for( uint32_t i = 0; i < size.value; ++i ) {
               auto tmp = ::fcl::detail::default_construct_pair_maybe_with_allocator<K, V>( value.get_allocator() );
               unpack( s, tmp );
               value.insert( value.end(), std::move(tmp) );
            }
         }
   ...
   namespace detail {
      ...
      void to_variant_from_set( const Set<T, U...>& s, fcl::variant& vo ) {
         FCL_ASSERT( s.size() <= MAX_NUM_ARRAY_ELEMENTS );
         variants vars;
         vars.reserve( s.size() );
      ...
      template<template<typename...> class Set, typename T, typename... U>
      void from_variant_to_set( const fcl::variant& v, Set< T, U... >& s ) {
      ...
      template<template<typename...> class Set, typename T, typename... U>
      void from_variant_to_flat_set( const fcl::variant& v, Set< T, U... >& s ) {
         const variants& vars = v.get_array();
         FCL_ASSERT( vars.size() <= MAX_NUM_ARRAY_ELEMENTS );
         s.clear();
         s.reserve( vars.size() );
      ...
      void to_variant_from_map( const Map< K, V, U... >& m, fcl::variant& vo ) {
         FCL_ASSERT( m.size() <= MAX_NUM_ARRAY_ELEMENTS );
         variants vars;
         vars.reserve( m.size() );
