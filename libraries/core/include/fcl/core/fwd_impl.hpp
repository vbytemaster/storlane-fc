...
namespace fcl {
    ...
    namespace detail {
      ...
      struct add {
        typedef decltype( *((A*)0) + *((typename fcl::remove_reference<U>::type*)0) ) type;
      };
      template<typename A, typename U>
      struct add_eq {
        typedef decltype( *((A*)0) += *((typename fcl::remove_reference<U>::type*)0) ) type;
    ...
    auto operator + ( const fwd<T,S,A>& x, U&& u ) -> typename detail::add<T,U>::type { return *x+fcl::forward<U>(u); }

    template<typename T, unsigned int S, typename U, typename A>
    auto operator - ( const fwd<T,S,A>& x, U&& u ) -> typename detail::sub<T,U>::type { return *x-fcl::forward<U>(u); }
    ...
    void check_size() { static_assert( (ProvidedSize >= RequiredSize), "Failed to reserve enough space in fcl::fwd<T,S>" ); }

    template<typename T,unsigned int S,typename A>
    template<typename... U>
    fwd<T,S,A>::fwd( U&&... u ) {
      check_size<sizeof(T),sizeof(_store)>();
      new (this) T( fcl::forward<U>(u)... );
    }
