...
/// mock out fcl::time_point::now() and provide a mock deadline timer
...
public:
   ...
   // Requires set_now() to be called first on main thread before any calls to fcl::time_point::now()
   static time_type now() noexcept;

   // First call should be on one thread before any calls to fcl::time_point::now()
   static void set_now( time_type t );
   static void set_now( const fcl::time_point& now );
   ...
   // return now as fcl::time_point, used by fcl::time_point::now() if mock_time_traits is_set()
   static fcl::time_point fcl_now();
