...
void mock_time_traits::set_now( time_type t ) {
   fcl::time_point now( fcl::microseconds( (t - epoch_).total_microseconds() ) );
   set_now( now );
}

void mock_time_traits::set_now( const fcl::time_point& now ) {
...
fcl::time_point mock_time_traits::fcl_now() {
   return fcl::time_point( fcl::microseconds( ( mock_time_traits::now() - epoch_ ).total_microseconds() ) );
}
