...
namespace fcl { namespace json_relaxed
{
   ...
   std::string quoteStringFromStream( T& in )
   {
       ...
       try
       {
           ...
           switch( q )
           {
               case '\'':
                   if( strict )
                       FCL_THROW_EXCEPTION( parse_error_exception, "expected: '\"' at beginning of string, got '\''" );
                   // falls through
               case '"':
                   break;
               default:
                   if( strict )
                       FCL_THROW_EXCEPTION( parse_error_exception, "expected: '\"' at beginning of string" );
                   else
                       FCL_THROW_EXCEPTION( parse_error_exception, "expected: '\"' | '\\\'' at beginning of string" );
           }
           if( in.peek() == q )
           {
               ...
               if( strict )
                   FCL_THROW_EXCEPTION( parse_error_exception, "triple quote unsupported in strict mode" );
               else
               {
                   in.get();
                   ...
                   while( true )
                   {
                       ...
                       if( c == q )
                       {
                           ...
                           FCL_THROW_EXCEPTION( parse_error_exception, "unexpected EOF in string '${token}'",
                                      ("token", token.str() ) );
           ...
           while( true )
           {
               ...
               if( c == q )
               {
                   ...
                   FCL_THROW_EXCEPTION( parse_error_exception, "unexpected EOL in string '${token}'",
                              ("token", token.str() ) );
               else
               {
                   in.get();
   ...
   std::string stringFromStream( T& in )
   {
      try
      {
         ...
         switch( c )
         {
             ...
             case 'r':
                 if( strict )
                     FCL_THROW_EXCEPTION( parse_error_exception, "raw strings not supported in strict mode" );
             case 'R':
                 in.get();
                 c2 = in.peek();
                 switch( c2 )
                 {
                     ...
                     default:
                         if( strict )
                             FCL_THROW_EXCEPTION( parse_error_exception, "unquoted strings not supported in strict mode" );
             ...
             case EOF:
                  FCL_THROW_EXCEPTION( eof_exception, "unexpected end of file" );
             default:
                 FCL_THROW_EXCEPTION( parse_error_exception, "expected: string" );
         }

       } FCL_RETHROW_EXCEPTIONS( warn, "while parsing string" );
   }
   ...
   fcl::variant parseInt( const std::string& token, size_t start )
   {
       ...
       if( i >= n )
           FCL_THROW_EXCEPTION( parse_error_exception, "zero-length integer" );
       ...
       while(true)
       {
           ...
           uint8_t vc = ctbl[c];
           if( vc == 0xFF )
               FCL_THROW_EXCEPTION( parse_error_exception, "illegal character ${c} in integer of base ${b}", ("c", c)("b", base) );
           if( val > maxb4mul )
               FCL_THROW_EXCEPTION( parse_error_exception, "integer literal overflow" );
           val *= base;
       ...
       if( token[0] == '-' )
       {
           if( val > INT64_MAX_PLUS_ONE )
               FCL_THROW_EXCEPTION( parse_error_exception, "negative integer literal overflow" );
           // special cased to avoid trying to compute -INT64_MIN which is probably undefined or something
           if( val == INT64_MAX_PLUS_ONE )
               return fcl::variant( INT64_MIN );
           return fcl::variant( -static_cast<int64_t>(val) );
       }
       return fcl::variant( val );
   }

   template<bool strict, uint8_t base>
   fcl::variant maybeParseInt( const std::string& token, size_t start )
   {
       try
       {
       ...
       catch( const parse_error_exception &e )
       {
           if( strict )
               ...
               return fcl::variant( token );
       }
   }

   template<bool strict>
   fcl::variant parseNumberOrStr( const std::string& token )
   { try {
       //ilog( (token) );
       size_t i = 0, n = token.length();
       if( n == 0 )
           FCL_THROW_EXCEPTION( parse_error_exception, "expected: non-empty token, got: empty token" );
       switch( token[0] )
       {
           case '+':
               if( strict )
                   FCL_THROW_EXCEPTION( parse_error_exception, "unary + not supported in strict mode" );
               i++;
               break;
           case '-':
               i++;
       ...
       switch( c )
       {
           case '0':
               if( i >= n )
                   return fcl::variant( uint64_t( 0 ) );
               switch( token[i] )
               {
                   case 'b':
                   case 'B':
                       if( strict )
                           FCL_THROW_EXCEPTION( parse_error_exception, "binary numeric literals not supported in strict mode" );
                       i++;
                       if( i >= n )
                           FCL_THROW_EXCEPTION( parse_error_exception, "empty binary numeric literal" );
                       return maybeParseInt<strict, 2>( token, i+1 );
                   case 'o':
                   case 'O':
                       if( strict )
                           FCL_THROW_EXCEPTION( parse_error_exception, "octal numeric literals not supported in strict mode" );
                       return maybeParseInt<strict, 8>( token, i+1 );
                   case 'x':
                   case 'X':
                       if( strict )
                           FCL_THROW_EXCEPTION( parse_error_exception, "hex numeric literals not supported in strict mode" );
                   ...
                   default:
                       ...
                       if( strict )
                           FCL_THROW_EXCEPTION( parse_error_exception, "expected '.'|'e'|'E' parsing number, got '${c}'",
                               ( "c", c ) );
               }
               break;
           ...
           case '_': case '-': case '.': case '+': case '/':
               if( strict )
                   FCL_THROW_EXCEPTION( parse_error_exception, "illegal character '${c}' parsing number", ( "c", c ) );
               return fcl::variant( token );
           default:
               FCL_THROW_EXCEPTION( parse_error_exception, "illegal character '${c}' in token", ( "c", c ) );
       }
       size_t start = i-1;
           ...
           switch( c )
           {
               ...
               case '.':
                   ...
                   if( dot_ok )
                   {
                       ...
                       {
                           if( strict )
                               FCL_THROW_EXCEPTION( parse_error_exception, "number cannot end with '.' in strict mode" );
                           return fcl::variant( fcl::to_double(token.c_str()) );
                       }
                       ...
                       switch( c )
                       {
                           ...
                           case 'E':
                               if( strict )
                                   FCL_THROW_EXCEPTION( parse_error_exception, "expected digit after '.'" );
                               break;
                           ...
                           case '_': case '-': case '.': case '+': case '/':
                               ...
                               return fcl::variant( token );
                           default:
                               FCL_THROW_EXCEPTION( parse_error_exception, "illegal character '${c}' in token", ( "c", c )("i",int(c)) );
                       }
                   }
                   else
                   {
                       if( strict )
                           FCL_THROW_EXCEPTION( parse_error_exception, "illegal multiple . in number" );
                       return fcl::variant( token );
                   }
                   break;
               case 'e':
               case 'E':
                   ...
                   {
                       if( strict )
                           FCL_THROW_EXCEPTION( parse_error_exception, "expected exponent after 'e'|'E' parsing number" );
                       return fcl::variant( token );
                   }
                   c = token[i++];
                   switch( c )
                   {
                       case '+': case '-':
                           ...
                           {
                               if( strict )
                                   FCL_THROW_EXCEPTION( parse_error_exception, "expected exponent" );
                               return fcl::variant( token );
                           }
                           break;
                       case '0': case '1': case '2': case '3': case '4':
                       ...
                       case '_':           case '.':           case '/':
                           if( strict )
                               FCL_THROW_EXCEPTION( parse_error_exception, "illegal character '${c}' in number", ( "c", c ) );
                           return fcl::variant( token );
                       default:
                           FCL_THROW_EXCEPTION( parse_error_exception, "illegal character '${c}' in token", ( "c", c ) );
                   }
   ...
   { try {
       ...
       if( strict && !(result.is_int64() || result.is_uint64() || result.is_double()) )
           FCL_THROW_EXCEPTION( parse_error_exception, "expected: number" );
       return result;
   ...
   variant wordFromStream( T& in )
   {
       ...
       FCL_THROW_EXCEPTION( parse_error_exception, "expected: null|true|false" );
   }
