...
inline void evp_digest_init(EVP_MD_CTX* ctx, const EVP_MD* md) {
   if (1 != EVP_DigestInit_ex(ctx, md, nullptr))
      FCL_THROW("error initializing EVP digest", fcl::error::ctx("code", static_cast<uint32_t>(ERR_get_error())));
}

inline void evp_digest_update(EVP_MD_CTX* ctx, const char* data, uint32_t size) {
   if (1 != EVP_DigestUpdate(ctx, data, size))
      FCL_THROW("error updating EVP digest", fcl::error::ctx("code", static_cast<uint32_t>(ERR_get_error())));
}
...
inline void evp_digest_final(EVP_MD_CTX* ctx, char* out, size_t expected_size) {
   ...
   if (1 != EVP_DigestFinal_ex(ctx, reinterpret_cast<unsigned char*>(out), &out_size))
      FCL_THROW("error finalizing EVP digest", fcl::error::ctx("code", static_cast<uint32_t>(ERR_get_error())));
   FCL_ASSERT(out_size == expected_size, "unexpected EVP digest size", fcl::error::ctx("out_size", out_size),
              fcl::error::ctx("expected_size", expected_size));
}
