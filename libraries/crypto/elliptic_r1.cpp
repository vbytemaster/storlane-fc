...
void throw_openssl_error(const char* message) {
   FCL_THROW("OpenSSL error", fcl::error::ctx("message", message),
...
public_key_data point_to_public_key_data(const EC_POINT* point) {
   ...
   FCL_ASSERT(written == result.size(), "unexpected R1 public key size", fcl::error::ctx("written", written),
...
public_key_point_data point_to_uncompressed_data(const public_key_data& data) {
   ...
   FCL_ASSERT(written == result.size(), "unexpected uncompressed R1 public key size",
              fcl::error::ctx("written", written), fcl::error::ctx("expected", result.size()));
   return result;
}
...
public_key_data derive_public_key_data(const private_key_secret& secret) {
   FCL_ASSERT(valid_secret(secret), "invalid R1 private key");
   const ec_group& group = get_curve();
   bn_ctx ctx(BN_CTX_new());
...
std::vector<unsigned char> sign_der(const private_key_secret& secret, const fcl::sha256& digest) {
...
std::optional<public_key_data> recover_public_key_from_sig(ECDSA_SIG* sig, const fcl::sha256& digest, int recid,
                                                           bool check) {
   const BIGNUM *r = nullptr, *s = nullptr;
   ECDSA_SIG_get0(sig, &r, &s);
   FCL_ASSERT(r != nullptr && s != nullptr, "invalid R1 signature");
   ...
   try {
      ...
      FCL_ASSERT(eor != nullptr, "error allocating R1 recovery bignums");
      FCL_ASSERT(EC_GROUP_get_order(group, order, ctx));
      ...
      FCL_ASSERT(R != nullptr, "error allocating R1 recovery point");
      ...
      if (check) {
         ...
         FCL_ASSERT(O != nullptr, "error allocating R1 recovery check point");
      ...
      FCL_ASSERT(Q != nullptr, "error allocating R1 recovered point");
...
compact_signature signature_from_ecdsa(const public_key_data& pub_data, fcl::ecdsa_sig& sig, const fcl::sha256& d) {
   const BIGNUM *sig_r = nullptr, *sig_s = nullptr;
   ECDSA_SIG_get0(sig, &sig_r, &sig_s);
   BIGNUM* r = BN_dup(sig_r);
   BIGNUM* s = BN_dup(sig_s);
   FCL_ASSERT(r != nullptr && s != nullptr, "error copying R1 signature bignums");
...
public_key public_key::mult(const fcl::sha256& digest) {
   const ec_group& group = get_curve();
   bn_ctx ctx(BN_CTX_new());
...
public_key public_key::add(const fcl::sha256& digest) const {
   try {
      const ec_group& group = get_curve();
      bn_ctx ctx(BN_CTX_new());
...
private_key private_key::regenerate(const fcl::sha256& secret) {
   private_key self;
   self.my->_key = secret;
   FCL_ASSERT(valid_secret(self.my->_key), "invalid R1 private key");
   return self;
}
...
signature private_key::sign(const fcl::sha256& digest) const {
   const auto der = sign_der(my->_key, digest);
   signature sig;
   std::memset(sig.data(), 0, sig.size());
   FCL_ASSERT(der.size() <= sig.size(), "R1 DER signature is too large", fcl::error::ctx("der_size", der.size()),
              fcl::error::ctx("signature_size", sig.size()));
   std::memcpy(sig.data(), der.data(), der.size());
   return sig;
}

bool public_key::verify(const fcl::sha256& digest, const fcl::crypto::r1::signature& sig) {
   if (is_empty(my->_key))
      return false;
...
fcl::sha512 private_key::get_shared_secret(const public_key& other) const {
   ...
   return fcl::sha512::hash(reinterpret_cast<const char*>(secret.data()), secret.size());
}
...
compact_signature private_key::sign_compact(const fcl::sha256& digest) const {
   try {
      FCL_ASSERT(valid_secret(my->_key));
