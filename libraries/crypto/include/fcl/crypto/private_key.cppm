...
class private_key {
 public:
   ...
   template <typename KeyType = ecc::private_key_shim> static private_key generate() {
      return private_key(storage_type(KeyType::generate()));
   }

   template <typename KeyType = r1::private_key_shim> static private_key generate_r1() {
      return private_key(storage_type(KeyType::generate()));
   }
 ...
 private:
   ...
   private_key(storage_type&& other_storage) : _storage(std::move(other_storage)) {}
