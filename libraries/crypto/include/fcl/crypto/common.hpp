...
namespace fcl { namespace crypto {
   ...
   struct base58_str_parser_impl<Result, Prefixes, Position> {
      static Result apply(const std::string& prefix_str, const std::string& data_str ) {
         FCL_ASSERT(false, "No matching suite type for ${prefix}_${data}", ("prefix", prefix_str)("data",data_str));
      }
   };
   ...
   struct base58_str_parser<std::variant<Ts...>, Prefixes> {
      static std::variant<Ts...> apply(const std::string& base58str) {
         ...
         FCL_ASSERT(pivot != std::string::npos, "No delimiter in data, cannot determine suite type: ${str}", ("str", base58str));

         const auto prefix_str = base58str.substr(0, pivot);
         auto data_str = base58str.substr(pivot + 1);
         FCL_ASSERT(!data_str.empty(), "Data only has suite type prefix: ${str}", ("str", base58str));
   ...
   struct base58str_visitor : public fcl::visitor<std::string> {
      explicit base58str_visitor( const fcl::yield_function_t& yield )
      : _yield(yield) {};
...
FCL_REFLECT_TEMPLATE((typename T), fcl::crypto::checksummed_data<T>, (data)(check) )
FCL_REFLECT_TEMPLATE((typename T), fcl::crypto::shim<T>, (_data) )
