...
export namespace fcl::crypto {
   ...
   struct checksummed_data {
      ...
      friend Stream& operator<<( Stream& s, const checksummed_data& value )
      {
      ...
      template<typename Stream>
      friend Stream& operator>>( Stream& s, checksummed_data& value )
      {
   ...
   struct base58_str_parser_impl<Result, Prefixes, Position> {
      static Result apply(const std::string& prefix_str, const std::string& data_str ) {
         FCL_ASSERT(false,
                    "No matching suite type",
                    fcl::error::ctx("prefix", prefix_str),
   ...
   struct base58_str_parser<std::variant<Ts...>, Prefixes> {
      static std::variant<Ts...> apply(const std::string& base58str) {
         ...
         FCL_ASSERT(pivot != std::string::npos, "No delimiter in data, cannot determine suite type: ${str}", fcl::error::ctx("str", base58str));
   ...
   struct shim {
      ...
      template<typename Stream>
      friend Stream& operator<<( Stream& s, const shim& value )
      {
      ...
      template<typename Stream>
      friend Stream& operator>>( Stream& s, shim& value )
      {
