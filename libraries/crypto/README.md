...
### Generate Random Bytes For A Seed
...
auto seed = std::array<char, 32>{};
fcl::rand_bytes(seed.data(), static_cast<int>(seed.size()));
```

`rand_bytes` is for entropy. Do not log the generated bytes; if you need to show
an identifier, derive and render a public fingerprint instead.
